...
namespace KingOfThePit
{
    public class Camera
    {
        ...
        public void Update(float elapsed)
        {
            ...
            dep.X = (int)Math.Ceiling(100f * delta.X * inertia * elapsed * KingOfThePit.meterToPixel);
            dep.Y = (int)Math.Ceiling(100f * delta.Y * inertia * elapsed * KingOfThePit.meterToPixel);

            Console.WriteLine(dep);

            dep /= KingOfThePit.meterToPixel;
        ...
        public Vector2 ConvertScreenToWorld(Vector2 location)
        {
            ...
            t = KingOfThePit.ptr.graphics.GraphicsDevice.Viewport.Unproject(t, SimProjection, SimView, Matrix.Identity);

            return new Vector2(t.X, t.Y) * KingOfThePit.meterToPixel;
        }

        public Vector2 ConvertWorldToScreen(Vector2 location)
        {
            Vector3 t = new Vector3(location * KingOfThePit.pixelToMeter, 0);
        ...
        public bool isInView(Vector2 position)
        {
            ...
            if (position.X >= -margin && position.X <= KingOfThePit.ptr.graphics.PreferredBackBufferWidth + margin
                &&
                position.Y >= -margin && position.Y <= KingOfThePit.ptr.graphics.PreferredBackBufferHeight + margin)
                return true;
