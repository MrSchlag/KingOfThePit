...
namespace KingOfThePit
{
    public class GraphicObj : IDisposable
    {
        ...
        {
            if (KingOfThePit.ptr.camera.isInView(position) && animations.Count > 0) // Permet de draw que les elements present ds la vue
        ...
        {
            Vector2 positionInScreen = KingOfThePit.ptr.camera.ConvertWorldToScreen(position);
